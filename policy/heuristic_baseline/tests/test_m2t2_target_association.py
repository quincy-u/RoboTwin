...
class M2T2TargetAssociationTest(unittest.TestCase):
    ...
    def test_predict_uses_standard_infer_and_filters_target_contacts(self) -> None:
        backend = object.__new__(RoboTwinM2T2Backend)
        backend.num_points = 4
        backend.num_runs = 1
        backend._seed = 4
        ...
        self.assertEqual(backend.model.infer_calls, 2)

        backend.reset(4)
        with self.assertRaisesRegex(
            NoObjectQueryFailure,
            "no grasp contacts on the segmented target",
        ):
            backend.predict(
                xyz,
                np.zeros((4, 3), dtype=np.float32),
